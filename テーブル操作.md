...
| バリデーション | カスタムメッセージを表示したい  | ☆1       | validates :カラム名, presence: { message: "カスタムメッセージ" } | -                    |
| バリデーション | 形式に応じたバリデーションを追加したい | ☆1   | validates :カラム名, format: { with: /\A[a-zA-Z]+\z/, message: "英字のみ" } | -                    |
...
| マイグレーション | 複雑な条件付きデフォルトを追加したい | ☆1       | -                                        | add_column :テーブル名, :カラム名, :string, default: -> { 'CURRENT_TIMESTAMP' } |
...
| セキュリティ  | ソルトを使ったパスワードハッシュを追加したい | ☆1       | -                                        | add_column :テーブル名, :salt, :string |
...
| スコープ    | 条件付きスコープを追加したい     | ☆1       | scope :recent, -> { where('created_at > ?', 1.week.ago) } | -                    |
...
| コールバック | トランザクション内で処理を実行したい | ☆1       | after_commit :メソッド名                   | -                    |
...
| 関連       | 自己関連付けを設定したい         | ☆1       | has_many :関連モデル, class_name: 'モデル名', foreign_key: '親モデル_id' | add_reference :テーブル名, :親モデル, foreign_key: { to_table: :テーブル名 } |
